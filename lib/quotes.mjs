...
function economicsScore(quote, minKnownTotal) {
...
export function analyzeQuotes(mission, supplierCandidates, conversations, { fxRates = [], now = new Date().toISOString() } = {}) {
  ...
  const readyConversations = conversations.filter(conversation => conversation.missionId === mission.id && (
    conversation.status === "offer_ready" || conversation.negotiation?.latestEvaluation?.status === "ready_for_comparison"
  ));
  if (!readyConversations.length) throw new Error("No supplier offers are ready for quote comparison");

  const quotes = readyConversations.map(conversation => {
    const candidate = candidateById.get(conversation.supplierId);
    if (!candidate) throw new Error(`Supplier candidate ${conversation.supplierId} is missing`);
  ...
  if (!eligible.length) {
    return {
      ...
      quotes,
      recommendation: null,
      blockers: ["No offer has enough normalized price/FX and technical evidence for ranking"]
    };
  }

...
export function qualifySupplier(mission, candidate, now = new Date().toISOString()) {
  ...
  const checks = {
    ...
    leadTime: candidate.leadTimeDays <= constraints.maxLeadTimeDays,
    moq: candidate.moq <= mission.quantity,
    commercialPlausibility: candidate.preliminaryUnitPrice <= mission.currentSupplier.unitPrice
  };
  ...
  if (candidate.leadTimeDays > constraints.maxLeadTimeDays + 5) hardFailures.push(`lead time exceeds the mission limit by ${candidate.leadTimeDays - constraints.maxLeadTimeDays} days`);
  if (candidate.moq > mission.quantity * 2) hardFailures.push("MOQ is incompatible with the requested quantity");
  ...
  if (!checks.leadTime) reviewFlags.push(`lead time is ${candidate.leadTimeDays - constraints.maxLeadTimeDays} day${candidate.leadTimeDays - constraints.maxLeadTimeDays === 1 ? "" : "s"} over target`);
  if (!checks.moq) reviewFlags.push("MOQ exceeds the requested quantity");
  if (!checks.commercialPlausibility) reviewFlags.push("preliminary price is above the current supplier");

  let status = "qualified";
  let reason = "Region, confidence, technical fit, MOQ, and lead time pass the mission screen; pricing is plausible for outreach and negotiation.";

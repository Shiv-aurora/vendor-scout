...
function normalizeReceivedAt(value) {
  ...
  if (typeof value !== "string" || !value.trim()) throw new Error("receivedAt must be a valid ISO timestamp or null");
  const parsed = Date.parse(value);
  if (!Number.isFinite(parsed)) throw new Error("receivedAt must be a valid ISO timestamp or null");
...
export function recordSupplierReply(conversation, { content, sourceReference, providerMessageId, receivedAt = null }) {
  ...
  if (providerMessageId != null && (typeof providerMessageId !== "string" || !providerMessageId.trim())) throw new Error("providerMessageId must be a non-empty string or null");
  ...
  conversation.messages.push({
    ...
    createdAt: normalizedReceivedAt
  });
  conversation.status = "supplier_replied";
  conversation.updatedAt = normalizedReceivedAt;
  return conversation;
}

async function readProviderJson(response) {
  const raw = await readBoundedText(response, MAX_PROVIDER_RESPONSE_BYTES, "Outreach provider");
  let payload = {};
  if (raw) {
...
async function sendWithRemoteProvider({ url, token, mission, candidate, conversation, message }) {
  ...
  if (payload.status == null || !String(payload.status).trim()) throw new Error("Outreach provider response is missing delivery status");

const MCP_SERVER_INFO = Object.freeze({ name: "vendor-scout", version: "0.6.0" });
...
const annotations = (readOnly, openWorld) => ({ readOnlyHint: readOnly, destructiveHint: false, idempotentHint: true, openWorldHint: openWorld });
...
export async function handleMcpMessage(message, context) {
  ...
  if (message.method === "initialize") {
    ...
    return rpcResult(message.id, {
      ...
      instructions: "Vendor Scout tools operate on persisted sourcing missions. Research, non-binding outreach, negotiation, and deterministic quote analysis may proceed autonomously. Supplier facts, offer terms, FX assumptions, and recommendation math must retain provenance. Quote analysis may recommend but cannot accept commercial terms, spend money, order samples, or place an order; those commitments remain human-gated and are not exposed by these tools."
    });
  }

...
const CANDIDATE_PROPERTIES = new Set(Object.keys(candidateSchema.properties));
...
export async function handleMcpMessage(message, context) {
  ...
  if (message.method === "initialize") {
    ...
    return rpcResult(message.id, {
      protocolVersion: typeof requestedVersion === "string" && SUPPORTED_PROTOCOL_VERSIONS.has(requestedVersion) ? requestedVersion : DEFAULT_PROTOCOL_VERSION,
  ...
  try {
    ...
    if (name === "vendor_scout_record_supplier_candidates") {
      validateCandidateBatch(args.candidates);
    ...
    if (name === "vendor_scout_record_supplier_reply") return rpcResult(message.id, toolResult(await context.recordReply(missionId, supplierIdFrom(args), {
      content: args.content,
      sourceReference: args.sourceReference,
      providerMessageId: Object.hasOwn(args, "providerMessageId") ? args.providerMessageId : null,
      receivedAt: Object.hasOwn(args, "receivedAt") ? args.receivedAt : null

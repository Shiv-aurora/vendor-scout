...
export function evaluateOffer(mission, candidate, offer, competitorOffers = [], { now = new Date().toISOString() } = {}) {
...
export function prepareCounter(mission, candidate, conversation, competitorOffers = [], options = {}) {
  const offer = latestOffer(conversation);
  if (!offer) throw new Error("No structured supplier offer has been recorded for this conversation");
  const evaluation = evaluateOffer(mission, candidate, offer, competitorOffers, options);

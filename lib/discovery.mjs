...
function normalizeCandidate(candidate, mission, index, sourceKind) {
  ...
  if (candidate.confidence < 0 || candidate.confidence > 1) throw new Error(`Discovery result ${index + 1} has confidence outside 0..1`);
  if (candidate.specMatch < 0 || candidate.specMatch > 1) throw new Error(`Discovery result ${index + 1} has specMatch outside 0..1`);
  if (candidate.preliminaryUnitPrice < 0 || candidate.moq < 0 || candidate.leadTimeDays < 0) throw new Error(`Discovery result ${index + 1} contains a negative commercial value`);
...
async function discoverFromRemoteProvider(mission, { url, token }) {
  ...
  if (payload.suppliers.length > MAX_REMOTE_CANDIDATES) throw new Error(`Discovery provider returned more than ${MAX_REMOTE_CANDIDATES} suppliers`);

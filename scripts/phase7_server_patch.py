...
negotiation = '''function competitorOffersFor(missionId, excludedSupplierId) {
...
'''
text = text.replace(anchor, negotiation + anchor, 1)

old_context = '''async function mcpRecordReply(id, supplierId, payload) {
...
new_context = '''async function mcpRecordReply(id, supplierId, payload) {
  return serializeMutation(() => recordMissionSupplierReply(id, supplierId, payload));
}
...
routes = '''    const offerMatch = url.pathname.match(/^\\/api\\/missions\\/([^/]+)\\/suppliers\\/([^/]+)\\/offer$/);
...
'''
text = text.replace(mission_route, routes + mission_route, 1)

path.write_text(text)
Path('.github/workflows/one-shot-phase7-server.yml').unlink(missing_ok=True)
Path('scripts/phase7_server_patch.py').unlink(missing_ok=True)
